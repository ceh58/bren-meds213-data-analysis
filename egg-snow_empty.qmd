...
```{r}
eggs_sum <- eggs_data %>%
  group_by(Year, Site, Nest_ID) %>%
  summarise(Egg_count = max(Egg_num)) %>%
  ungroup()
```
...
nest_egg_sum <- eggs_sum %>%
  left_join(nest_data, by = c("Year", "Site", "Nest_ID"))
```
...
nest_species_avg <- nest_egg_sum %>%
  group_by(Species) %>%
  summarise(Avg_egg_count = mean(Egg_count))
...
nest_species_avg <- nest_species_avg %>%
  left_join(species_data, by = c("Species" = "Code")) %>%
  select(-c(Relevance, Common_name))
```
...
```{r}
nest_species_avg %>% 
  relocate(Avg_egg_count, .after = last_col()) %>%
...
```{r}
snowcover_data_fixed2 <- snowcover_data_fixed %>%
  mutate(Date2 = ifelse(is.na(Date2), ymd("2006-06-08"), Date))

snowcover_data_fixed2 %>% filter(Date2 == "8&9 june 06")
```
